...
order_date_by_customer <-
  ...
  summarise(order_date = min(order_date),
            total_price = sum(cost_of_part * invoiced_qty_shipped,
                              na.rm = TRUE)
            ) %>% 
  ungroup()  
```
...
order_date_by_customer %>% 
  ...
  group_by(customer_id,
         ...
         customer_region,
         top_customer_group) %>% 
  select(order_date, order_num, total_price) %>% 
  distinct() %>% 
...
avg_diff_order_date_customers %>% 
  ...
  labs(
    x = "Number of orders",
    y = "Customer region",
    title = "Distribution of number of orders per customer is right-skewed"
  ) + 
  scale_x_log10()
``` 
