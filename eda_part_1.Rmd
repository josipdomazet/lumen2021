...
order_date_by_customer %>% 
  ...
  mutate(avg_diff = as.numeric(avg_diff)) -> avg_diff_order_date_customers

```
 
Split in equally sized bins
```{r}
avg_diff_order_date_customers %>% 
  mutate(avg_diff = as.numeric(avg_diff)) %>% 
  mutate(group = cut_number(avg_diff, n = 10)) %>% 
  view("zanimljivo")
```
...
avg_diff_order_date_customers %>% 
  ...
  labs(
    x = "Average difference of two consecutive order dates in days"
  )
```
