...
dat %>%
  ...
  filter_var_percentile(revenue, 0.001) %>%  
  ggplot(aes(revenue)) +
  geom_histogram(
    bins = 50,
    fill = hue_pal()(2)[2], 
...
dat %>%
  ...
  filter_var_percentile(invoiced_qty_shipped, 0.01) %>% 
  sample_frac(0.1) %>% 
...
dat %>% 
  ...
  mutate(price_bin = case_when(
    ...
    filter_var_percentile(cost_of_part, 0.01) %>% 
...
dat %>% 
  ...
  mutate(profit = (invoiced_qty_shipped *
                            (invoiced_price - cost_of_part))) %>% 
  filter_var_percentile(profit, 0.05) %>% 
